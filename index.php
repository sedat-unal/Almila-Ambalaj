...
<body>
    ...
    <section class="banner-bottom">
        <div class="container">
            ...
            <div class="row inner-sec-wthree">
                <?php
                $query2 = $dbh->query('select top_fade_right, top_fade_left from index_tr');
                while ($row2 = $query2->fetch()) {
                ?>
                    <div class="col-lg-5 bt-bottom-info" data-aos="fade-right">
                        <h5><?= $row2['top_fade_right'] ?></h5>
                    </div>
                    <div class="col-lg-7 bt-bottom-info" data-aos="fade-left">
                        <p><?= $row2['top_fade_left'] ?></p>
                    </div>
                <?php
                }
                ?>
            ...
            <div class="row inner-sec-wthree">
                <?php
                $query3 = $dbh->query('select icon, subTitle, content, href from index_icons1 where status = 1');
                while ($row3 = $query3->fetch()) {
                ?>
                    <div class="col-lg-4 bottom-sub-grid text-center aos-init aos-animate" data-aos="zoom-in">
                        <div class="bt-icon">

                            <span class="<?= $row3['icon'] ?>"></span>
                        </div>

                        <h4 class="sub-tittle"><?= $row3['subTitle'] ?></h4>
                        <p><?= $row3['content'] ?></p>
                        <p>
    ...
    <!-- /stats -->
    <?php
    $count = $dbh->query('select count(*) from index_icons2 where status = 1')->fetchColumn();
    if ($count > 1) {
    ?>
            ...
            <div class="row inner_stat">
                <div class="col-lg-6 stats-left-info">
                    <div class="row">
                        <?php
                        $i = 0;
                        $query4 = $dbh->query('select icon,number,title from index_icons2 where status = 1');
                        while ($row4 = $query4->fetch()) {
                        ?>
                            <div class="col-md-6 stats_left counter_grid<?= $i != 0 ? '' . $i : "" ?>">
                                <i class="<?= $row4['icon'] ?>"></i>
                                <p class="counter"><?= $row4['number'] ?></p>
                                <h4><?= $row4['title'] ?></h4>
                            </div>
                        <?php
                            $i++;
    ...
    ?>
        <section class="services">
            <div class="container">
                <h3 class="tittle">Neden Almila Ambalaj ?</h3>
                <div class="row inner-sec-wthree">
    ...
    <!---->
    <?php
    $count = $dbh->query('select count(*) from index_down where status = 1')->fetchColumn();
    if ($count > 1) {
    ?>
        <section class="grids_sec_2">
            <div class="row style-grids_main">
                <?php
                $i = 0;
                $q = $dbh->query('select title, text, img from index_down where status = 1');
                while ($row = $q->fetch()) {
                    if ($i % 2 == 0) {
                ?>
                        ...
                        <div class="col-md-6" style="background: -webkit-linear-gradient(rgba(23, 22, 23, 0.2), rgba(23, 22, 23, 0.5)), url(../images/<?= $row['img'] ?>) no-repeat;
                        background-size: cover;
                        min-height: 600px;">
                        </div>
                    <?php
    ...
    <!--/Projects-->
    <?php
    $count = $dbh->query('select count(*) from index_gallery where status = 1')->fetchColumn();
    if ($count > 1) {
    ?>
        <section class="services">
            <div class="container">
                <h3 class="tittle">Son Projelerimiz
                </h3>
                <div class="row inner-sec-wthree">
                    <?php
                    $query = $dbh->query('select title, img, description, content from index_gallery where status = 1');
                    while ($row = $query->fetch()) {
                    ?>
